...
PREPROCESSED_UNIT_DATA = read_csv_from_preprocessed_data(
    "postcodes_impute_postcode_method.csv"
)
PREPROCESSED_UNIT_DATA_KNN = read_csv_from_preprocessed_data(
    "postcodes_impute_KNN.csv"
)
PREPROCESSED_SECTOR_DATA = read_csv_from_preprocessed_data(
    "sector_preprocessed.csv"
)
PREPROCESSED_DISTRICT_DATA = read_csv_from_preprocessed_data(
    "district_imputed.csv"
)
PREPROCESSED_RAINFALL_STATION = read_csv_from_preprocessed_data(
    "rainfall_stations.csv"
)
PREPROCESSED_LEVEL_STATION = read_csv_from_preprocessed_data(
    "level_stations_imputed.csv"
)
PREPROCESSED_WET_DAY = read_csv_from_example_data("wet_day_preprocessed.csv")
PREPROCESSED_TYPICAL_DAY = read_csv_from_example_data(
    "typical_day_preprocessed.csv"
)
...
class Tool(object):
    ...
    def __init__(
        ...
        district_prep : DataFrame, optional
            DataFrame containing information on households by postcode
            district, preprocessed.

        rainfall_stations : DataFrame, optional
            DataFrame containing information on rainfall stations,
            preprocessed.

        level_stations : DataFrame, optional
            DataFrame containing information on water level stations,
            preprocessed.
        ...
        for model in models:
            if update_hyperparameters:
                ...
                if model == "random_classification":
                    ...
                    model_instance = SevenClassTool(label_col="riskLabel").fit(
                        self.unit_data
                    )
                    ...
                    merge_sector = unit_sector_merge(
                        self.unit_prep, self.sector_prep
                    )
                    ...
                    from .models.historic_flood_classifier import (
                        build_historic_rf,
                    )
                    ...
                    self.models["knn_authority"] = KNNLocalAuthorityModel(
                        n_neighbors=5
                    ).fit(X_loc, self.unit_data["localAuthority"])
                    self.models["knn_risk"] = KNNFloodRiskModel(
                        n_neighbors=15
                    ).fit(X_loc, self.unit_data["riskLabel"])
    ...
    def lookup_lat_long(
        ...
        DF = pd.DataFrame(
            {"longitude": lons, "latitude": lats}, index=postcodes, dtype=dtype
        )
        return DF

    def get_postcode_district_sector(
        self, postcodes: Sequence
    ) -> pd.DataFrame:
        """
        get_postcode_district_sector(postcodes)

        Get a Pandas dataframe containing formatted postcodes, postcode
        districts, and postcode sectors for sequence of postcodes of any
        format.
    ...
    def get_station_coordinates(self, dataframe: pd.DataFrame) -> pd.DataFrame:
        ...
        Get a Pandas dataframe containing longitude, latitude, easting and
        northing coordinates of rainfall and water level stations based on
        stationReference.
        ...
        for i in range(len(DF)):
            ...
            if DF["parameter"][i] == "rainfall":
                station_row = self.rainfall_stations[
                    self.rainfall_stations["stationReference"] == station_ref
                ].reset_index()
            elif DF["parameter"][i] == "level":
                station_row = self.level_stations[
                    self.level_stations["stationReference"] == station_ref
                ].reset_index()
            ...
            # np.nan if station cannot be found in database
                ...
                lons[i], lats[i], easts[i], norths[i] = (
                    np.nan,
                    np.nan,
                    np.nan,
                    np.nan,
    ...
    def handle_outliers_iqr(
        self, df: pd.DataFrame, cols: str | list[str]
    ) -> pd.DataFrame:
        """
        Handle outliers in one or more DataFrame columns using the
        Interquartile Range (IQR) method.
        Outliers are capped at lower and upper bounds derived from the 1st
        quartile (Q1), 3rd quartile (Q3), and IQR (1.5*IQR rule).
        ...
        cols : str or list of str
            Single column name (str) or list of column names to handle outliers
            for.

        Returns
        -------
        pandas.DataFrame
            A copy of the input DataFrame with outliers in specified column(s)
            capped. Original DataFrame remains unmodified.
        ...
        >>> import pandas as pd
        >>> df = pd.DataFrame(
        ...     {'a': [1, 2, 3, 100, -50], 'b': [5, 6, 7, 200, -100]}
        ... )
        >>> # Handle single column
        >>> cleaned_single = tool.handle_outliers_iqr(df, 'a')
        >>> # Handle multiple columns
        >>> cleaned_multi = tool.handle_outliers_iqr(
        ...     df, ['a', 'b'])  # doctest: +SKIP
        """
    ...
    def impute_missing_values(
        ...
        method : str, optional
            ...
            * ``'postcode'``, impute using median (numerical features) or
              mode (categorical features) of other datapoints within the same
              postcode sector or postcode district in the labelled dataset.
            * ``'knn'``, impute using median (numerical features) or mode
              (categorical features) of the k-nearest neighbours based on
              geographic coordinates from the labelled dataset.
            * ``'constant'``, impute using a constant value for imputation.

        constant_values : dict, optional
            Dictionary containing constant values to use for imputation in
            the format ``{column_name: value}``.
        ...
        columns_impute_num = list(
            DF[columns_impute].select_dtypes(include=[np.number]).columns
        )
        columns_impute_cat = list(
            DF[columns_impute].select_dtypes(exclude=[np.number]).columns
        )

        # Postcode method
        if method == "postcode":
            # First get postcode sectors and postcode districts
            unit_data["postcodeSector"] = self.get_postcode_district_sector(
                unit_data["postcode"]
            )["postcodeSector"]
            unit_data["postcodeDistrict"] = self.get_postcode_district_sector(
                unit_data["postcode"]
            )["postcodeDistrict"]
            DF["postcodeSector"] = self.get_postcode_district_sector(
                DF["postcode"]
            )["postcodeSector"]
            DF["postcodeDistrict"] = self.get_postcode_district_sector(
                DF["postcode"]
            )["postcodeDistrict"]
            #
            # Numerical columns
            if len(columns_impute_num) > 0:
                # Medians per sector/district from labelled dataset
                sector_medians = unit_data.groupby("postcodeSector")[
                    columns_impute_num
                ].median()
                district_medians = unit_data.groupby("postcodeDistrict")[
                    columns_impute_num
                ].median()
            ...
            if len(columns_impute_cat) > 0:
                # Modes per sector/district from labelled dataset
                sector_modes = (
                    unit_data.dropna(subset=columns_impute_cat)
                    .groupby("postcodeSector")[columns_impute_cat]
                    .agg(lambda x: x.mode().iloc[0])
                )
                district_modes = (
                    unit_data.dropna(subset=columns_impute_cat)
                    .groupby("postcodeDistrict")[columns_impute_cat]
                    .agg(lambda x: x.mode().iloc[0])
                )
        ...
        # knn method
            ...
            if len(columns_impute_num) > 0:
                for col in columns_impute_num:
                    ...
                    for i in range(len(DF)):
                        if pd.isna(vals[i]):
                            # Nearest neighbours in labelled dataset
                            dists, indexes = neighbors.kneighbors(
                                [coords_new[i]]
                            )
                            neighbor_vals = pd.Series(
                                unit_data[col][indexes[0]]
                            ).dropna()
                            # Impute with median of k neighbours
                            if len(neighbor_vals) > 0:
                                vals[i] = np.nanmedian(neighbor_vals)
                            # Else impute with median of entire dataset
                            else:
                ...
                for col in columns_impute_cat:
                    ...
                    for i in range(len(DF)):
                        if pd.isna(vals[i]):
                            ...
                            # Else use default imputation constants
                            else:
    ...
    def impute_district_property_age(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Impute missing district-level property age values for rows with all
        null targets.
        ...
        ----------
        df : pandas.DataFrame
            District DataFrame with required feature/target columns (outliers
            handled).

        Returns
        -------
        pandas.DataFrame
            District DataFrame with imputed values for rows missing all
            targets.
        ...
        --------
        >>> tool = Tool()
        >>> df_imputed = tool.impute_district_property_age(
        ...     df)   # doctest: +SKIP
        """
        ...
        df_test_pred = pd.DataFrame(
            predictions, columns=targets, index=df_test.index
        )
        df_test_pred = df_test_pred.div(
            df_test_pred.sum(axis=1), axis=0
    ...
    def predict_flood_class_from_OSGB36_location(
        ...
        if model == "all_minimum_risk":
            ...
            lat, lon = get_gps_lat_long_from_easting_northing(
                df["easting"].values, df["northing"].values, dms=False
            )
            ...
            if "knn_risk" not in self.models:
                raise ValueError(
                    "Model 'knn' not trained. Call fit_to_data(['knn']) first."
                )
    ...
    def predict_median_house_price(
        ...
        if model == "all_england_median":
            ...
            if merge_sector.shape[0] == 0:
                ...
                X = self.house_proc_pipe.transform(
                    merge_sector.drop(columns=["medianPrice"])
                )
    ...
    def predict_high_risk_near_watercourses(
        ...
        if df.empty:
            return pd.DataFrame(
                columns=["postcode", "riskLabel"] + list(df.columns)
            )

        # Flood risk level per location via OSGB36 prediction
        easting = df["easting"].tolist()
        northing = df["northing"].tolist()
        risk_series = self.predict_flood_class_from_OSGB36_location(
            easting, northing
        )
    ...
    def predict_local_authority(
        ...
        if model == "modal_local_authority":
            ...
            return pd.Series(
                data=predictions, index=idx, name="localAuthority"
            )
    ...
    def estimate_total_value(self, postal_data: Sequence[str]) -> pd.Series:
        ...
        valid_sectors = sector_data[sector_data["households"].notna()]
        sector_households_map = valid_sectors.set_index("postcodeSector")[
            "households"
        ].to_dict()
        ...
        for postcode in postal_data:
            ...
            sector = self.get_postcode_district_sector([postcode]).iloc[0][
                "postcodeSector"
            ]
    ...
    def estimate_annual_human_flood_risk(
        ...
        # Build mapping relationship
        sector_headcount = sector_data.set_index("postcodeSector")[
            "headcount"
        ].to_dict()
